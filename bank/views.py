...
import random
from .models import account,history,vrf,email_token,otp_save
...
from . thread import class_email_otp,class_money_otp,class_forget_password,class_send_money_sender,class_send_money_resever
...
def email_otp(request):
    if request.user.is_authenticated:
        ...
        #     fail_silently=False
        # )
        class_email_otp(dt.email,o).start()
            ...
            if account.objects.filter(acn=acnd).exists():
                with transaction.atomic():
                    if ot == otp:
                        ...
                        class_send_money_resever(resever_email.email, user_email.first_name, user_email.last_name, amonut, acnd, time, 'Done').start()
                        class_send_money_sender(user_email.email,resever_email.first_name,resever_email.last_name,amonut,user_obj.acn,time,'Done').start()
...
def money_otp(request):
    if request.user.is_authenticated:
        if account.objects.filter(user=request.user).exists():
            ...
            o = g * 5
            # otp(mail,o)
            class_money_otp(mail,o).start()
            messages.success(request,'OTP send your register email id.....')
            return render(request, 'sendmoney.html',{'o':o})
        else:
...
def forget_password(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=user).exists():
            ...
            class_forget_password(user_obj.email,token,user_obj.username).start()
